 ...
 * <p>
 * Calls made to the request happen on the requestOn executor The callback is executed on the
 * requestOnExecutor The responded passed to the call back passes its value to a responder defined
 * with respondWith and on the respondOn executor.
 *
...
class AsyncRequest<T> extends AbstractRequest<T> {
    ...
    /**
     * Async Request does not provide a default cancel method. One should be set using
     * {@link Request#cancelWith(Canceller)}. The reason no method is provided is because async
     * calls are more likely to spin off their calling thread, and the default method of
     * interrupting the thread they were called on is likely to do nothing.
     */
    @Override
    public void cancel() {
     ...
     * most of the usual tasks after a value is calculated. Since Async task receives this value in
     * a callback, we need to provide a callback to consume this value.
     *
        ...
        private void afterAsyncCalledComplete() {
            try {
                ...
                if (responderThrowable instanceof Error) {
                    /*
                     * Applications should not try to handle errors. If a responder threw an error
                     * this means that something fundamental has failed and it should be passed out
                     * of the Reactive system.
                     */
                    throw responderThrowable;
                } else {
                    /*
                     * Responders with uncaught exceptions should not blow up the reactive stack.
                     * For now we will log them, but one day a RxPlugin style mechanism may be
                     * appropriate.
                     */
