...
SecurityCheckResult result = securityService.checkAndSendMetric(SecurityCheckType.IS_ROOTED, metricsService);
----

Metrics information containing the results of the checks run via an Executor will be automatically sent if the Executor is supplied an instance of the `MetricsService`:

[source, java]
----
MetricsService metricsService = mobileCore.getInstance(MetricsService.class);
SyncSecurityCheckExecutor syncCheckExecutor = SecurityCheckExecutor.Builder.newSyncExecutor(this.getContext())
    .withSecurityCheck(SecurityCheckType.IS_ROOTED)
    // other checks...
    .withMetricsService(metricsService)
    .build();

// The returned results have already been queued for reporting via the Metrics Service
