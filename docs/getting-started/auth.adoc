...
`AuthService` can be retrived using the `MobileCore#getInstance` method.
...
To retrieve the current authenticated user the `AuthService#currentUser` method can be invoked.
...
if (currentUser) {
    // User is already authenticated, perform an action
} else {
