...
public class OIDCAuthenticatorImplTest {
    ...
    private static final String OIDC_RESPONSE =
                    "{\"request\":{\"configuration\":{\"authorizationEndpoint\":\"https:\\/\\/keycloak.security."
    ...
    private static final String AUTH_STATE =
                    "{\"refreshToken\":\"eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJhZFNveVhOQWdReFY0M2VxSFNpUl"
    ...
    private String logoutURL = String.format(
                    "https://keycloak.security.feedhenry.org/auth/realms/secure-app/protocol/openid-connect/logout?id_token_"
                                    + "hint=%s&redirect_uri=org.aerogear.mobile.example:/callback",
                    identityToken);
    ...
    public void setup() {
        ...
        when(serviceConfig.getProperty("url")).thenReturn("https://keycloak-server.com/auth");
        ...
        when(keycloakConfiguration.getLogoutUrl("test-identity-token",
                        "some.redirect.uri:/callback")).thenReturn(logoutURL);

        doAnswer(invocation -> {
            ((AuthorizationService.TokenResponseCallback) invocation.getArguments()[1])
                            .onTokenRequestCompleted(tokenResponse, null);
            return null;
        }).when(authorizationService).performTokenRequest(any(TokenRequest.class),
        ...
        userPrincipalImpl = UserPrincipalImpl.newUser().withUsername("test-user")
